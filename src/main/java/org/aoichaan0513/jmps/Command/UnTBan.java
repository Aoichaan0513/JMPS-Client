...
public class UnTBan implements CommandExecutor {
    JavaPlugin plugin;

    public UnTBan(JavaPlugin plugin) {
        this.plugin = plugin;
    }
                    ...
                    if (!CoreAPI.isOnlinePlayer(args[0])) {
                        ...
                        if (CoreAPI.isPlayed(op)) {
                            if (CoreAPI.isPunished(op, CoreAPI.PunishType.TEMPORARY)) {
                                ...
                                String result = CoreAPI.sendPost("UnTBan.php", json);

                                try {
                                    JSONObject jsonObject = new JSONObject(result);
                                    if (jsonObject.getBoolean("result")) {
                                        sender.sendMessage(CoreAPI.getSuccessPrefix() + op.getName() + "の期限Banを解除しました。");
                                        return true;
                                    } else {
                                        sender.sendMessage(CoreAPI.getErrorPrefix() + op.getName() + "の期限Banを解除できませんでした。\n" +
                            ...
                            sender.sendMessage(CoreAPI.getErrorPrefix() + op.getName() + "はすでにこのサーバーで期限Banされていません。");
                            return true;
                        }
