...
public class UnLBan implements CommandExecutor {
    JavaPlugin plugin;

    public UnLBan(JavaPlugin plugin) {
        this.plugin = plugin;
    }
                    ...
                    if (!CoreAPI.isOnlinePlayer(args[0])) {
                        ...
                        if (CoreAPI.isPlayed(op)) {
                            if (CoreAPI.isPunished(op, CoreAPI.PunishType.LOCAL)) {
                                ...
                                String result = CoreAPI.sendPost("UnLBan.php", json);

                                try {
                                    JSONObject jsonObject = new JSONObject(result);
                                    if (jsonObject.getBoolean("result")) {
                                        sender.sendMessage(CoreAPI.getSuccessPrefix() + op.getName() + "のローカルBanを解除しました。");
                                        return true;
                                    } else {
                                        sender.sendMessage(CoreAPI.getErrorPrefix() + op.getName() + "のローカルBanを解除できませんでした。\n" +
                            ...
                            sender.sendMessage(CoreAPI.getErrorPrefix() + op.getName() + "はすでにこのサーバーでローカルBanされていません。");
                            return true;
                        }
