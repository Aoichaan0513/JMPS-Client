                        ...
                        if (CoreAPI.isOnlinePlayer(args[0])) {
                            ...
                            if (CoreAPI.isPlayed(p)) {
                                if (!CoreAPI.isPunished(p, CoreAPI.PunishType.GLOBAL)) {
                                    ...
                                    String result = CoreAPI.sendPost("GBan.php", json);

                                    try {
                                        JSONObject jsonObject = new JSONObject(result);
                                        if (jsonObject.getBoolean("result")) {
                                            p.kickPlayer(ChatColor.RED + "あなたはグローバルBanされています。\n" +
                                                    ChatColor.GRAY + "処罰理由: " + str.trim() + "\n" +
                                                    ChatColor.GRAY + "処罰者: " + sender.getName());
                                            sender.sendMessage(CoreAPI.getSuccessPrefix() + p.getName() + "をグローバルBanしました。\n" +
                                                    CoreAPI.getInfoPrefix() + "処罰理由: " + str.trim());
                                            return true;
                                        } else {
                                            sender.sendMessage(CoreAPI.getErrorPrefix() + p.getName() + "をグローバルBanできませんでした。\n" +
                                ...
                                sender.sendMessage(CoreAPI.getErrorPrefix() + p.getName() + "はすでにグローバルBanされています。");
                                return true;
                            }
                            ...
                            if (CoreAPI.isPlayed(op)) {
                                if (!CoreAPI.isPunished(op, CoreAPI.PunishType.GLOBAL)) {
                                    ...
                                    try {
                                        JSONObject jsonObject = new JSONObject(result);
                                        if (jsonObject.getBoolean("result")) {
                                            sender.sendMessage(CoreAPI.getSuccessPrefix() + op.getName() + "をグローバルBanしました。\n" +
                                                    CoreAPI.getInfoPrefix() + "処罰理由: " + str.trim());
                                            return true;
                                        } else {
                                            sender.sendMessage(CoreAPI.getErrorPrefix() + op.getName() + "をグローバルBanできませんでした。\n" +
                                                    CoreAPI.getInfoPrefix() + "理由: " + jsonObject.getString("reason"));
                                            return true;
                                        }
                                    } catch (Exception e) {
                                ...
                                sender.sendMessage(CoreAPI.getErrorPrefix() + op.getName() + "はすでにグローバルBanされています。");
                                return true;
                            }
