...
public class LBan implements CommandExecutor {
    JavaPlugin plugin;

    public LBan(JavaPlugin plugin) {
        this.plugin = plugin;
    }
                        ...
                        if (CoreAPI.isOnlinePlayer(args[0])) {
                            ...
                            if (CoreAPI.isPlayed(p)) {
                                if (!CoreAPI.isPunished(p, CoreAPI.PunishType.LOCAL)) {
                                    ...
                                    String result = CoreAPI.sendPost("LBan.php", json);

                                    try {
                                        JSONObject jsonObject = new JSONObject(result);
                                        if (jsonObject.getBoolean("result")) {
                                            p.kickPlayer(ChatColor.RED + "あなたはローカルBanされました。\n" +
                                                    ChatColor.GRAY + "処罰理由: " + str.trim() + "\n" +
                                                    ChatColor.GRAY + "処罰者: " + sender.getName());
                                            sender.sendMessage(CoreAPI.getSuccessPrefix() + p.getName() + "をローカルBanしました。\n" +
                                                    CoreAPI.getInfoPrefix() + "処罰理由: " + str.trim());
                                            return true;
                                        } else {
                                            sender.sendMessage(CoreAPI.getErrorPrefix() + p.getName() + "をローカルBanできませんでした。\n" +
                                ...
                                sender.sendMessage(CoreAPI.getErrorPrefix() + p.getName() + "はすでにこのサーバーでローカルBanされています。");
                                return true;
                            }
                            ...
                            if (CoreAPI.isPlayed(op)) {
                                if (!CoreAPI.isPunished(op, CoreAPI.PunishType.LOCAL)) {
                                    ...
                                    try {
                                        JSONObject jsonObject = new JSONObject(result);
                                        if (jsonObject.getBoolean("result")) {
                                            sender.sendMessage(CoreAPI.getSuccessPrefix() + op.getName() + "をローカルBanしました。\n" +
                                                    CoreAPI.getInfoPrefix() + "処罰理由: " + str.trim());
                                            return true;
                                        } else {
                                            sender.sendMessage(CoreAPI.getErrorPrefix() + op.getName() + "をローカルBanできませんでした。\n" +
                                                    CoreAPI.getInfoPrefix() + "理由: " + jsonObject.getString("reason"));
                                            return true;
                                        }
                                    } catch (Exception e) {
                                ...
                                sender.sendMessage(CoreAPI.getErrorPrefix() + op.getName() + "はすでにこのサーバーでローカルBanされています。");
                                return true;
                            }
