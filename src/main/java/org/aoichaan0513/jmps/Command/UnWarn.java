...
public class UnWarn implements CommandExecutor {
    ...
    public UnWarn(JavaPlugin plugin) {
        this.plugin = plugin;
    }
                    ...
                    if (CoreAPI.isOnlinePlayer(args[0])) {
                        ...
                        if (CoreAPI.isPlayed(op)) {
                            if (CoreAPI.isPunished(op, CoreAPI.PunishType.WARNING)) {
                                ...
                                String result = CoreAPI.sendPost("UnWarn.php", json);

                                try {
                                    JSONObject jsonObject = new JSONObject(result);
                                    if (jsonObject.getBoolean("result")) {
                                        sender.sendMessage(CoreAPI.getSuccessPrefix() + op.getName() + "の警告を解除しました。");
                                        return true;
                                    } else {
                                        sender.sendMessage(CoreAPI.getErrorPrefix() + op.getName() + "の警告を解除できませんでした。\n" +
                            ...
                            sender.sendMessage(CoreAPI.getErrorPrefix() + op.getName() + "はすでにこのサーバーで警告されていません。");
                            return true;
                        }
